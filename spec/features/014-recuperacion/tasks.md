...
- [x] T1 — Servicios + schemas en lib/auth.ts + tests de servidor
  ...
  - solicitarRecuperacion(email válido) → no lanza y el correo aparece en
    Mailpit (consultar API de Mailpit en el test).
  - solicitarRecuperacion(email inexistente) → no lanza y NO aparece correo
    en Mailpit; mismo comportamiento observable (mensaje genérico).
  - restablecerPassword del flujo completo: obtener link de Mailpit →
...
- [x] T2 — Route handler callback + Server Actions
  ...
  ver el link en Mailpit → abrirlo → cambiar → login).

- [x] T3 — Páginas + componentes + link en /login
...
- [x] T4 — E2E Playwright
  ...
  Helper (inline o e2e/helpers.ts): leer correos de Mailpit
  GET http://127.0.0.1:54324/api/v1/search?query=to:<email> +
  /api/v1/message/<id>, esperar (poll con
  ...
  - Flujo completo en un test() (cookies compartidas): /login → link
    ...
    enviado → leer link de Mailpit → goto link (callback intercambia code) →
